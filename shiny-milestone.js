...
function useIssues(issues) {
  ...
  _.each(issues, function(issue) {
    if (issue && issue.milestone && issue.milestone.title == config.milestone) {
      ...
      fs.appendFileSync(config.name, _.template('<span class="issue-body" style="display: none"><pre><img class="usr-img" data-placement="right" rel="tooltip" title="<%= user.login %>" src="<%= user.avatar_url %>">\n\n<%= body %></pre></span>')(issue));
